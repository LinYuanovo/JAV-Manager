...
def scan_movies(root: str, only_scan = False, data_cache_file = "") -> List[Movie]:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ignore_folders = [f.strip() for f in cfg.File.ignore_folder.split(';') if f.strip()]
        for name in dirnames.copy():
            if name.startswith('.') or name in ignore_folders:
                dirnames.remove(name)
        for file in filenames:
            ext = os.path.splitext(file)[1].lower()
            # media_ext 格式如 "mp4;mkv" 不带点，需要去掉 ext 的点后匹配
            if ext[1:] in cfg.File.media_ext.split(';'):
