...
def parse_data(movie: MovieInfo):
    ...
    container_tag = html.xpath("//div[@class='container']")
    if not container_tag:
        raise WebsiteError(f"javbus: 页面缺少 container 元素 - {movie.dvdid}")
    container = container_tag[0]
    title_tag = container.xpath("h3/text()")
    if not title_tag:
        raise WebsiteError(f"javbus: 页面缺少标题 - {movie.dvdid}")
    title = title_tag[0]
    cover_tag = container.xpath("//a[@class='bigImage']/img/@src")
    if not cover_tag:
        raise WebsiteError(f"javbus: 页面缺少封面图 - {movie.dvdid}")
    cover = cover_tag[0]
    preview_pics = container.xpath("//div[@id='sample-waterfall']/a/@href")
    info_tag = container.xpath("//div[@class='col-md-3 info']")
    if not info_tag:
        raise WebsiteError(f"javbus: 页面缺少信息栏 - {movie.dvdid}")
    info = info_tag[0]
    dvdid_tag = info.xpath("p/span[text()='識別碼:']")
    if not dvdid_tag:
        raise WebsiteError(f"javbus: 页面缺少識別碼 - {movie.dvdid}")
    dvdid = dvdid_tag[0].getnext().text
    publish_date_tag = info.xpath("p/span[text()='發行日期:']")
    if not publish_date_tag:
        raise WebsiteError(f"javbus: 页面缺少發行日期 - {movie.dvdid}")
    publish_date = publish_date_tag[0].tail.strip()
    duration_tag = info.xpath("p/span[text()='長度:']")
    if not duration_tag:
        raise WebsiteError(f"javbus: 页面缺少时长 - {movie.dvdid}")
